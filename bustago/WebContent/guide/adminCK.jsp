...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("img#ck").click(function() {

			//alert($("input[id=bus_no]").val());
			//sub_no=($("input[id=bus_no]").val().substr(0,10));
			//alert('a');
			$("form#ckSubmit").submit();
		...
		<div class="container bg5">
			<div class="location">
				<a href="/kr/index.do" class="home">HOME</a> &gt; <a
					href="/kr/support/notice/notice.do">관리자모드</a> &gt; <a class="state">1:1문의내역</a>
			</div>
				...
				<div class="lnbs" id="Lnb">
					...
					<div class="slnb">
						<ul class="lnb" id="support">
							<li><a href="/bustagoApp/AdminMemberAction.action">회원관리</a></li>
							<li><a href="/guide/question.jsp">결제관리</a></li>
							<li><a href="/bustagoApp/AdminQuestionAction.action">1:1
									문의 내역</a></li>
							<li><a href="/bustagoApp/AdminNoticeAction.action">공지사항관리</a></li>
						</ul>
					</div>
					<!-- //lnb -->
				</div>
					...
					<table class="board_list" summary="번호,제목,첨부,등록일,조회수">
						...
						<thead>
							<tr>
								...
								<th scope="col">카드유효기간</th>
								<th scope="col">결제금액</th>
								<th scope="col">예매 승인 여부</th>
								<th scope="col">승인처리</th>
							</tr>
						</thead>
						<tbody>
							<form action="/bustagoApp/ckFlagAction.action?y_no=${i.y_no}"
								name="ckSubmit" id="ckSubmit" method="post">
								<c:forEach var="i" items="${cklist}" varStatus="cnt">
									<tr>
										<td>${cnt.count}</td>
										<td>${i.y_no}</td>
										<td>${i.m_id}</td>
										...
										<td>${i.total_pay}</td>
										<td>${i.yeyak_ck}</td>
										<td><img src="/images/button/btn_ok.gif" width="100"
											id="ck" name="ck" value="${i.y_no}" /></td>
									</tr>
								</c:forEach>
							</form>
						</tbody>
